...
def add_line(article: str, number: str) -> None: 
    ...
    files = [ 
        ...
        {"path": ADMIN_PATH / "index.html",
        "line_to_add": ADMIN_LINE},
    
        #file to edit for guess
        {"path": MAIN_PATH / "index.html",
        "line_to_add": GUEST_LINE}
    ]
        ...
        with open(new, 'w') as new_file:
            with open(home_path, 'r') as hfile:
                ...
                for line in hfile.readlines():
                    new_file.write(line)
                    if '<ul class="article_list">' in line: #beginning of the list of articles
...
def remove_line(article_title: str, number: str | int) -> None:
    ...
    article_line = f'<h2><a href="/pages/articles/{number}.json">{article_title}</a></h2>'
    files = [
...
def edit_page(article_file: str) -> bytes:
    ...
    lines_group = [
        ...
        # form lines
        ('<form>',
         f'\t    <form action="{article_file.name}" method="post">\n'),
...
def edit_article(article: dict, file: str):
    ...
    if not isinstance(article, dict):
        raise TypeError(f"Error: invalid argument of type: {type(article)} must ba 'dict'.")
    ...
    if old_title != article["title"]:
        ...
        home_pages = [ADMIN_PATH / "index.html",
                      MAIN_PATH / "index.html"]
